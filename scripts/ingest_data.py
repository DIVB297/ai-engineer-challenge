...
class DataIngester:
    ...
    async def send_batch_to_embedding_service(
        self, session: aiohttp.ClientSession, chunks: List[Dict]
    ):
        """Send a batch of chunks to the embedding service"""
        payload = {"documents": [{"id": chunk["id"], "text": chunk["text"]} for chunk in chunks]}
