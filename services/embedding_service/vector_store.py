...
class VectorStore:
    def __init__(self, connection_string: str, database_name: str, collection_name: str):
        ...
        if "mongodb.net" in connection_string or "mongodb+srv" in connection_string:
            ...
            # Local MongoDB
            self.client = AsyncIOMotorClient(
                connection_string, serverSelectionTimeoutMS=5000, connectTimeoutMS=10000
            )
            self.is_atlas = False
    ...
    async def initialize(self):
        ...
        try:
            ...
            if self.is_atlas:
                logger.info(
                    "Atlas detected - vector search index will be created on first embedding"
                )
    ...
    async def _ensure_vector_index(self, embedding_dimensions: int):
        ...
        try:
            ...
            if self.embedding_dimensions != embedding_dimensions:
                logger.warning(
                    f"Embedding dimensions changed from {self.embedding_dimensions} "
                    f"to {embedding_dimensions}"
                )
                self.embedding_dimensions = embedding_dimensions
            ...
            logger.info(
                f"MongoDB Atlas Vector Search initialized with {embedding_dimensions} dimensions"
            )
        except Exception as vector_error:
            logger.warning(
                f"Vector search index creation failed (will use fallback): {vector_error}"
            )
            self.vector_search_available = False

    async def upsert_embedding(
        self, doc_id: str, text: str, embedding: List[float], metadata: dict = None
    ):
        ...
        try:
            ...
            result = await self.collection.replace_one({"id": doc_id}, document, upsert=True)
            ...
            for doc in documents:
                try:
                    ...
                    result = await self.collection.replace_one(
                        {"id": doc["id"]}, document, upsert=True
                    )
        ...
        try:
            ...
            if similarity_metric == "cosine":
                ...
                pipeline = [
                    ...
                    {"$sort": {"score": -1}},
                    {"$limit": k},
                    {"$project": {"id": 1, "text": 1, "metadata": 1, "score": 1}},
                ]
            else:
            ...
            logger.info(
                f"Vector search completed with {similarity_metric}, found {len(results)} results"
            )
            return results
