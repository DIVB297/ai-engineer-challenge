...
from models import BulkEmbedRequest, EmbedRequest
...
def mock_vector_store():
    ...
    store.similarity_search = AsyncMock(
        return_value=[{"id": "test_1", "text": "Test document", "score": 0.95, "metadata": {}}]
    )
    return store
...
class TestVectorStore:
    ...
    async def test_vector_store_initialization(self):
        with patch("vector_store.AsyncIOMotorClient"):
